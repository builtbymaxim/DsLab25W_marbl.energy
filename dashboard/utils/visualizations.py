...
# --- Color Constants ---
# MARBL brand colors (from marbl.energy website)
MARBL_PRIMARY = "#00B4BC"      # Teal - primary accent
MARBL_PRIMARY_DARK = "#009AA2" # Darker teal - hover states
MARBL_SECONDARY = "#31CEDC"    # Soft cyan - gradients
MARBL_NAVY = "#0B253F"         # Dark navy - text
MARBL_GREY = "#6D7C8A"         # Medium grey - secondary text

# Consistent color scheme for zones (aligned with marbl palette)
ZONE_COLORS = {
    "DK1": "#00B4BC",  # Teal - Wind (Denmark)
    "ES": "#F39C12",   # Amber - Solar (Spain)
    "NO2": "#31CEDC"   # Cyan - Hydro (Norway)
}

# Color palette for clusters (teal-based with complementary colors)
CLUSTER_COLORS = [
    "#00B4BC",  # Teal (primary)
    "#0B253F",  # Navy
    "#31CEDC",  # Cyan
...
# Common Plotly layout settings for marbl branding
MARBL_LAYOUT = dict(
    font=dict(family="Inter, sans-serif", color=MARBL_NAVY),
    title_font=dict(family="Montserrat, sans-serif", size=18, color=MARBL_NAVY),
    paper_bgcolor="white",
    plot_bgcolor="white",
    hoverlabel=dict(
        bgcolor="white",
    ...
    if show_std and "price_std" in profile_df.columns:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                fill="toself",
                fillcolor="rgba(0, 180, 188, 0.15)",  # Marbl teal with transparency
...
def create_cluster_calendar_heatmap(
    ...
    # Custom teal-based colorscale for marbl branding
    # Starting with visible grey (not white) for contrast against white background
    marbl_colorscale = [
        [0.0, "#B8C5D0"],   # Medium grey - visible against white bg
        [0.2, "#7DD3D8"],   # Light cyan
        [0.4, "#31CEDC"],   # Cyan
        [0.6, "#00B4BC"],   # Teal (primary)
        [0.8, "#009AA2"],   # Dark teal
        [1.0, "#0B253F"]    # Navy
    ]
...
def create_cluster_probability_bar(
    ...
    fig.update_layout(
        ...
        margin=dict(l=100, r=20, t=40, b=40),  # More left margin for labels
        **MARBL_LAYOUT
    )
