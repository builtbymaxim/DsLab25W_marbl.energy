...
Description: Day-ahead price forecast page with model predictions.
Author: MARBL Dashboard Team
Date: 2026-01-16

Note: This page uses real weather forecasts from WeatherAPI.
      Price predictions are still mock until XGBoost models are trained.
"""
...
def main():
    ...
    with st.spinner("Generating forecast..."):
        price_forecast = generate_mock_forecast(selected_zone, forecast_datetime)
        cluster_probs = generate_mock_cluster_probabilities(selected_zone, forecast_datetime)

        # Load fresh price data (last 2 days for context)
        recent_actual, last_price_timestamp = load_fresh_prices(selected_zone, days=2)

        # Filter to show only 48h before forecast start for cleaner visualization
    ...
    status_parts.append("Forecast: mock (awaiting ML models)")
    ...
    with col_cluster2:
        ...
        cluster_descriptions = {
            1: "Low price day - typically high renewable generation (wind/solar)",
            2: "Morning peak pattern - high demand in business hours",
            3: "Evening peak pattern - residential demand surge after sunset",
            4: "Stable flat profile - balanced supply and demand",
            5: "High volatility day - supply constraints or weather variability"
        }

        for cluster_id, prob in sorted(cluster_probs.items(), key=lambda x: -x[1]):
            if prob > 0.1:
                st.markdown(
                    f"- **Cluster {cluster_id}** ({prob:.1%}): {cluster_descriptions.get(cluster_id, 'Unknown pattern')}"
                )
    ...
    st.markdown("---")
    st.caption(
        "Forecasts are model predictions for educational purposes only. "
